 ...
 * @subpackage  Auth
 */
class Auth_Login_Cmsauth extends \Auth\Auth_Login_Driver {
    ...
    public static function _init() {
        ...
        if (\Config::get('cmsauth.remember_me.enabled', false)) {
            static::$remember_me = \Session::forge(array(
                        'driver' => 'cookie',
                        'cookie' => array(
                            'cookie_name' => \Config::get('cmsauth.remember_me.cookie_name', 'rmcookie'),
                        ),
                        'encrypt_cookie' => true,
                        'expire_on_close' => false,
                        'expiration_time' => \Config::get('cmsauth.remember_me.expiration', 86400 * 31),
            ));
        }
    }
    ...
    protected $config = array(
        'drivers' => array('group' => array('Cmsgroup')),
    ...
    protected function perform_check() {
        ...
        if (!empty($username) and ! empty($login_hash)) {
            if (is_null($this->user) or ( $this->user['username'] != $username and $this->user != static::$guest_login)) {
                $this->user = \DB::select_array(\Config::get('cmsauth.table_columns', array('*')))
                                ->where('username', '=', $username)
                                ->from(\Config::get('cmsauth.table_name'))
                                ->execute(\Config::get('cmsauth.db_connection'))->current();
            }

            // return true when login was verified, and either the hash matches or multiple logins are allowed
            if ($this->user and ( \Config::get('cmsauth.multiple_logins', false) or $this->user['login_hash'] === $login_hash)) {
                return true;
            }
        }
        ...
        $this->user = \Config::get('cmsauth.guest_login', true) ? static::$guest_login : false;
    ...
    public function validate_user($username_or_email = '', $password = '') {
        $username_or_email = trim($username_or_email) ? : trim(\Input::post(\Config::get('cmsauth.username_post_key', 'username')));
        $password = trim($password) ? : trim(\Input::post(\Config::get('cmsauth.password_post_key', 'password')));
        ...
        $user = \DB::select_array(\Config::get('cmsauth.table_columns', array('*')))
                        ->where_open()
    ...
    public function create_user($username, $password, $email, $group = 20, Array $profile_fields = array(), $post_data) {
        ...
        //var_dump($user);
        //exit;
        $result = \DB::insert(\Config::get('cmsauth.table_name'))
                ->set($user)
                ->execute(\Config::get('cmsauth.db_connection'));
    ...
    public function update_user($values, $username = null) {
        ...
        $current_values = \DB::select_array(\Config::get('cmsauth.table_columns', array('*')))
                ->where('username', '=', $username)
                ->from(\Config::get('cmsauth.table_name'))
                ->execute(\Config::get('cmsauth.db_connection'));
        ...
        $affected_rows = \DB::update(\Config::get('cmsauth.table_name'))
                ->set($update)
                ->where('username', '=', $username)
                ->execute(\Config::get('cmsauth.db_connection'));
    ...
    public function delete_user($username) {
        ...
        $affected_rows = \DB::delete(\Config::get('cmsauth.table_name'))
                ->where('username', '=', $username)
                ->execute(\Config::get('cmsauth.db_connection'));
    ...
    public function create_login_hash() {
        ...
        $login_hash = sha1(\Config::get('cmsauth.login_hash_salt') . $this->user['username'] . $last_login);

        \DB::update(\Config::get('cmsauth.table_name'))
    ...
    public function get_groups() {
        ...
        return array(array('Cmsgroup', $this->user['group']));
    }
    ...
    public function guest_login() {
        return \Config::get('cmsauth.guest_login', true);
    }

}

// end of file cmsauth.php
